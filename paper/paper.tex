...
To allow predictions for fixed time-steps ahead, data is aggregated at a fixed resolution. \Cref{fig:resolutions} shows examples of travel time for a single link and a single day at various time resolutions. The black dots are actual measurements, and the lines the aggregated link travel time at the given resolution. The choice depends of the \emph{expected} frequency of the line, and is a balance between capturing the details and still having a reasonal number of measurements in each time-step.
